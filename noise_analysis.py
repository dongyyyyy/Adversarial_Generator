...
def make_noise_analysis(args):
    epsilon = 0.1
    threshold = 0.8
    img_save_path = f'/home/eslab/dataset/make_image_toAnalysis_noise_withSign/epsilon_{epsilon}/threshold_{threshold}/'
    os.makedirs(img_save_path,exist_ok=True)
    make_noise_number = 100
    number_of_class = 10
    ...
    for class_num in range(number_of_class):
        for index in range(make_noise_number):
            current_img_save_path = f'{img_save_path}{class_num}/{index}/'
            ...
            cv2.imwrite(f'{current_img_save_path}origin.png',generated_image)
            print(f'finish {current_img_save_path}origin.png')
            ...
            while(True):
                ...
                if predict[0,class_num] > threshold:
                    ...
                    save_img = generated_image.detach().clone().squeeze(0)
                    ...
                    save_img = (save_img * 255).cpu()
                    save_img = save_img.type(torch.uint8)
                    save_img = save_img.numpy()
                    save_img = save_img.transpose(1,2,0)
                    cv2.imwrite(f'{current_img_save_path}iter({number_of_iter}).png',save_img)
                    print(f'finish {current_img_save_path}iter({number_of_iter}).png')
                    number_of_iter += 1 
            ...
            cv2.imwrite(f'{current_img_save_path}final.png',generated_image)
            print(f'finish {current_img_save_path}final.png')

        
def make_noise_analysis_usingPascalVOC(args):
    threshold = 0.8
    epsilon = 0.1
    img_save_path = f'/home/eslab/dataset/make_image_toAnalysis_noise_withSign_pascal/epsilon_{epsilon}/threshold_{threshold}/'
    os.makedirs(img_save_path,exist_ok=True)
    make_noise_number = 100
    ...
    for class_num in range(number_of_class):
        ...
        for image in load_dataset:
            ...
            cv2.imwrite(f'{current_img_save_path}origin.png',save_init_image)
            print(f'finish {current_img_save_path}origin.png')
            indexing = 0
            while(True):
                batch_label = torch.tensor([class_num],dtype=torch.long).to(device)
                ...
                if predict[0,class_num] > threshold:
                    ...
                    cv2.imwrite(f'{current_img_save_path}{indexing}.png',save_init_image)
                    print(f'finish {current_img_save_path}{indexing}.png')
                indexing += 1
            ...
            cv2.imwrite(f'{current_img_save_path}final.png',save_init_image)
            print(f'finish {current_img_save_path}final.png')
            index += 1
